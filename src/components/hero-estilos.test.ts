 ...
 * única regla que envuelve a trazo, aplicador y «STUDIO»). Que además valga EXACTAMENTE 15 lo
 ...
 * verdad (el contrato exige que todo tiempo derive del token, no literales de 15 duplicados).
 */
...
/**
 * @s4 — REESCRITO (enmienda 2026-07-23, la CALIGRAFÍA LENTA) y RE-MEDIDO dos veces (ENMIENDA 3:
 * 90 s → 30 s; ENMIENDA 4, 2026-07-24, banco de vivencias iteración 2: 30 s → 15 s, el punto
 * dulce — ≈1,7 s por letra × 9 letras ≈ 15 s de trazo). El número vive en UN token
 * (`--duracion-caligrafia: 15s`) y TODO tiempo del rótulo deriva de él — el reloj del timeout de
 * Hero.tsx lo asevera `hero-logica.test.ts` contra estos MISMOS bytes. La curva sigue `linear`:
 * también a 15 s el cubic-bezier anterior reptaría en los extremos y correría en el centro; una
 * pluma escribe a velocidad constante. jsdom NO anima: BYTES del SCSS; el ritmo real, EN VIVO.
 */
describe('@s4 un ÚNICO token --duracion-caligrafia: 15s gobierna TODOS los tiempos del rótulo', () => {
  it('@s4 el token vale EXACTAMENTE 15s, vive en la regla raíz .escena y se declara UNA sola vez', () => {
    // El 15 va ESCRITO A MANO: la decisión de Pablo (≈1,7 s por letra × 9 letras), 2026-07-24.
    expect(reglaBase('escena')).toMatch(/--duracion-caligrafia\s*:\s*15s\s*;/)
    ...
    expect(segundosDelToken()).toBe(15)
  })

  it('@s4 fuera del token NO queda ninguna duración de trazo suelta: todo literal restante es ≤ 1 s', () => {
    // Si alguien duplicara el 15 (u otro trazo largo) como literal en una animación, dejaría DOS
    // relojes que divergen al editar uno. Comentarios fuera: los tiempos de prosa no son CSS.
    const sinComentarios = scss().replace(/\/\/[^\n]*/g, '')
    const sinToken = sinComentarios.replace(/--duracion-caligrafia\s*:\s*15s\s*;/, '')
  ...
  it('@s4 tinta y aplicador siguen siendo UN reloj: misma duración, mismo retardo, misma curva', () => {
    ...
    expect(escribir.duracion).toBe(15)
  ...
  it('@s4 «retirarse» arranca en calc(var(--duracion-caligrafia) - 0.4s + 0.1s), derivado del token', () => {
    ...
    // el token: cambiar los 15 s recoloca la retirada sola. La fórmula va ESCRITA A MANO.
    expect(reglaAnimada('aplicador')).toMatch(
      /retirarse\s+0\.5s\s+linear\s+calc\(var\(--duracion-caligrafia\)\s*-\s*0\.4s\s*\+\s*0\.1s\)\s+both/,
    )
    expect(tiemposDe('aplicador', 'retirarse').retardo).toBeCloseTo(14.7, 6)
  })
  ...
  it('@s4 «revelarStudio» arranca en calc(var(--duracion-caligrafia) + 0.2s) y «aparecer» sigue en 0.3s', () => {
    ...
    expect(tiemposDe('heroStudio', 'revelarStudio').retardo).toBeCloseTo(15.2, 6)
 ...
 */
describe('@s4 «STUDIO» entra CUANDO la marca ya está escrita, nunca antes — total ≈15,8 s', () => {
  it('@s4 el retardo de .heroStudio es ≥ que el final del trazo (retardo + duración)', () => {
    ...
    // En MILISEGUNDOS ENTEROS: en coma flotante 0,1 + 15 puede dar …0000006 y un `>=` fallaría
  ...
  it('@s4 la ceremonia completa (retardo + duración de «revelarStudio») suma 15,8 s EXACTOS', () => {
    const studio = tiemposDe('heroStudio', 'revelarStudio')
    const ms = (s: number) => Math.round(s * 1000)

    // 15 + 0,2 + 0,6 = 15,8 — el 15800 va ESCRITO A MANO y es el MISMO número que asevera
    // `hero-logica.test.ts` para el timeout de Hero.tsx: un solo reloj, dos vigilantes.
    expect(ms(studio.retardo + studio.duracion)).toBe(15_800)
  })
})
